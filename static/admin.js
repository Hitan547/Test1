...
function showDashboard(name, email) {
    ...
    document.getElementById('dashName').textContent = capitalized;
    document.getElementById('dashAvatar').textContent = capitalized.substring(0, 2).toUpperCase();

    if (window.innerWidth <= 768) {
        document.getElementById('menuToggle').style.display = 'flex';
    }

    // Load opportunities on dashboard init
    loadOpportunities();
}
...
function buildOpportunityCard(opp) {
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            <span class="applicants-count">${opp.max_applicants ? escapeHtml(opp.max_applicants) + ' max applicants' : 'Open applicants'}</span>
            <div style="display:flex;gap:8px;">
                <button class="view-course-btn btn-view" style="width:auto;padding:8px 14px;">View</button>
                <button class="view-course-btn btn-edit" style="width:auto;padding:8px 14px;background:#f0faf5;color:#2d7a50;border:1px solid #2d7a50;">Edit</button>
                <button class="view-course-btn btn-delete" style="width:auto;padding:8px 14px;background:#fff5f5;color:#dc2626;border:1px solid #dc2626;">Delete</button>
            </div>
        </div>
    `;
    ...
    card.querySelector('.btn-view').addEventListener('click', () => {
        openOpportunityDetails(opp.name, {
            ...
            skills:               skills,
            applicants:           opp.max_applicants || '—',
            futureOpportunities:  opp.future_opps,
...
async function loadOpportunities() {
    ...
    try {
        ...
        if (!res.ok) {
            grid.innerHTML = '<p style="color:#dc2626;padding:20px;">Failed to load opportunities.</p>';
            return;
        }
        ...
        if (data.length === 0) {
            grid.innerHTML = `
                <div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:#6b7280;">
                    <svg viewBox="0 0 24 24" style="width:48px;height:48px;stroke:#d1d5db;fill:none;stroke-width:1.5;margin-bottom:12px;display:block;margin-left:auto;margin-right:auto;">
                        <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/>
                        <rect x="9" y="3" width="6" height="4" rx="1"/>
                    </svg>
                    <p style="font-size:16px;font-weight:500;margin-bottom:6px;">No opportunities yet</p>
        ...
        grid.innerHTML = '<p style="color:#dc2626;padding:20px;">Network error. Please refresh and try again.</p>';
    }
}
...
// Handle opportunity form submission (create + edit)
document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (isEdit) {
            ...
            const oldCard = grid.querySelector(`[data-id="${editingOppId}"]`);
            if (oldCard) oldCard.replaceWith(buildOpportunityCard(data));
            showToast('Opportunity updated successfully!');
        } else {
            // Remove empty state if present, prepend new card
            const emptyState = grid.querySelector('div[style*="grid-column"]');
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        setTimeout(() => showDashboard(data.name, data.email), 800);
        generateCaptcha('login');
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 409) {
                ...
                showError('signupNameErr', data.error || 'Signup failed. Please try again.');
            }
        ...
        showError('signupNameErr', 'Network error. Please try again.');
        shakeForm('signupForm');
    }
});
